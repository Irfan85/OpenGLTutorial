...
void CreateTriangle()
{
	...
	// Now we want any openGL function that we call and buffer to use this particular VAO
	...
	// Now we're putting our vertices data into the VBO. Here, GL_STATIC_DRAW enum indidates we will not be changing any of the vertices data later
	...
	// After we're done with our VAO and VBO, We will release them. '0' means we're binding to nothing
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
}
...
int main()
{
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		// Clear window
		// We will clear the window but with a color, in this case its red. Hence this RGBA combo. the 'glClear' function will use these values
